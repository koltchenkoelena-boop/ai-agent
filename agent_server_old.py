...
class AgentServer:
    ...
    def _setup_routes(self):
        ...
        async def websocket_chat(websocket: WebSocket, session_id: str):
            ...
            print(f"[{self.name}] WS connected: session={session_id}")
            ...
            try:
                while True:
                    ...
                    msg = json.loads(data)
                    print(f"[{self.name}] WS message: {msg['content'][:50]}...")
                    ...
                    try:
                        ...
                        print(f"[{self.name}] Stream error: {error_msg}")
                        await websocket.send_text(
                ...
                print(f"[{self.name}] WS disconnected: session={session_id}")
        ...
        async with httpx.AsyncClient(timeout=300) as client:
            async with client.stream(
                ...
                if resp.status_code != 200:
                    ...
                    raise Exception(f"LLM API returned {resp.status_code}: {body[:200].decode()}")
